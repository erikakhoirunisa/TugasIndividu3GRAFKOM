...
void Demo::Update(double deltaTime) {
	angle += (float)((deltaTime * 1.5f) / 1200);
}
...
void Demo::BuildKiri()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("2_specular.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		...
		// front
		-0.85, -0.5, 0.5, 0, 0, 0.0f,  0.0f,  1.0f, // 0
		0.85, -0.5, 0.5, 1, 0,  0.0f,  0.0f,  1.0f, // 1
		0.85,  0.5, 0.5, 1, 1,  0.0f,  0.0f,  1.0f, // 2
		-0.85,  0.5, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3

		 // right
		-0.85,  1,  0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 4
		-0.85,  1, -0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 5
		 -0.85, -0.5, -0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 6
		 -0.85, -0.5,  0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 7

		// back
		-0.85, -0.5, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 8 
		0.885,  -0.5, -0.5,5, 5, 0.0f,  0.0f,  -1.0f, // 9
		0.85,   1, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 10
		-0.85,  1, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 11

		 // left
		 -0.5, -0.5, -0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 12
		 -0.5, -0.5,  0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 13
		 -0.5,  0.5,  0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 14
		 -0.5,  0.5, -0.5,5, 5, -1.0f,  0.0f,  0.0f, // 15

		// upper
		0.5, 0.5,  0.5, 5, 5,   0.0f,  1.0f,  0.0f, // 16
		-0.5, 0.5, 0.5, 5, 5,   0.0f,  1.0f,  0.0f, // 17
		-0.5, 0.5, -0.5, 5, 5,  0.0f,  1.0f,  0.0f, // 18
		0.5, 0.5, -0.5, 5, 5,   0.0f,  1.0f,  0.0f, // 19

		// bottom
		-0.5, -0.5, -0.5, 5, 5, 0.0f,  -1.0f,  0.0f, // 20
		0.5, -0.5, -0.5, 5, 5,  0.0f,  -1.0f,  0.0f, // 21
		0.5, -0.5,  0.5, 5, 5,  0.0f,  -1.0f,  0.0f, // 22
		-0.5, -0.5,  0.5, 5, 5, 0.0f,  -1.0f,  0.0f, // 23
	};
...
void Demo::BuildKanan()
{
	...
	float vertices[] = {
		// format position, tex coords
		// front
		0.82, -0.5, 0.5, 0, 0, 0.0f,  0.0f,  1.0f, // 0
		2.5, -0.5, 0.5, 1, 0,  0.0f,  0.0f,  1.0f, // 1
		2.5,  0.5, 0.5, 1, 1,  0.0f,  0.0f,  1.0f, // 2
		0.82,  0.5, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3

		 // right
		 0.87,  0.5,  0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 4
		 2.5,  0.5, -0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 5
		 2.5, -0.5, -0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 6
		 0.87, -0.5,  0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 7

		// back
		0.5, -0.5, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 8 
		2.5,  -0.5, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 9
		2.5,   1, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 10
		0.5,  1, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 11

		 // left
		2.5, -0.5, -0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 12
		 2.5, -0.5,  0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 13
		2.5,  1,  0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 14
		 2.5,  1, -0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 15

		// upper
		2.5, 0.5,  0.5, 5, 5,   0.0f,  1.0f,  0.0f, // 16
		0.5, 0.5, -0.5, 5, 5,   0.0f,  1.0f,  0.0f, // 17
		0.5, 0.5,  -0.5, 5, 5,  0.0f,  1.0f,  0.0f, // 18
		2.5, 0.5,  -0.5, 5, 5,  0.0f,  1.0f,  0.0f, // 19

		// bottom
		0.5, -0.5, -0.5, 5, 5, 0.0f,  -1.0f,  0.0f, // 20
		2.5, -0.5, -0.5, 5, 5,  0.0f,  -1.0f,  0.0f, // 21
		2.5, -0.5,  0.5, 5, 5,  0.0f,  -1.0f,  0.0f, // 22
		0.5, -0.5,  0.5, 5, 5,  0.0f,  -1.0f,  0.0f, // 23
	};
...
void Demo::BuildAtas()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("lemariatas_specular.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		...
		-0.85, 0.5, 0.5, 0, 0, 0.0f,  0.0f,  1.0f, // 0
		2.5, 0.5, 0.5, 1, 0,  0.0f,  0.0f,  1.0f, // 1
		2.5,  1, 0.5, 1, 1,  0.0f,  0.0f,  1.0f, // 2
		-0.85,  1, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3

		 // right
		 0.85,  0.5, 0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 4
		 2.5,  0.5,-0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 5
		 2.5, 0, -0.5, 5, 5, 1.0f,  0.0f,  0.0f, // 6
		 0.85, 0,  0.5, 5, 5, 1, 1.0f,  0.0f,  0.0f, // 7

		// back
		-0.85, 1, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 8 
		2.5,  1, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 9
		2.5, -0.5, -0.5, 5, 5,  0.0f,  0.0f,  -1.0f, // 10
		-0.85, -0.5, -0.5, 5, 5, 0.0f,  0.0f,  -1.0f, // 11

		 // left
		 -0.75, -0.5, -0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 12
		 -0.75, -0.5,  0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 13
		 -0.75,  1,  0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 14
		 -0.75,  1, -0.5, 5, 5, -1.0f,  0.0f,  0.0f, // 15

		// upper
		2.5, 1,  0.5, 5, 5,    0.0f,  1.0f,  0.0f, // 16
		-0.85,1,  0.5, 5, 5,    0.0f,  1.0f,  0.0f, // 17
		-0.85,1,-0.5, 5, 5,  0.0f,  1.0f,  0.0f, // 18
		2.5, 1, -0.5, 5, 5,   0.0f,  1.0f,  0.0f, // 19

		// bottom
		-0.85, -0.5, -0.5, 5, 5, 0.0f,  -1.0f,  0.0f, // 20
		2.5, -0.5, -0.5, 5, 5, 0.0f,  -1.0f,  0.0f, // 21
		2.5, -0.5,  0.5, 5, 5,  0.0f,  -1.0f,  0.0f, // 22
		-0.85, -0.5,  0.5, 5, 5, 0.0f,  -1.0f,  0.0f, // 23
	};
...
int main(int argc, char** argv) {
	...
	app.Start("Tugas 3 = Ligthing", 800, 600, false, false);
}